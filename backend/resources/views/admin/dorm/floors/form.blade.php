...
<form method="POST" action="{{ $floor->exists ? route('admin.dorm.floors.update', $floor) : route('admin.dorm.floors.store') }}">
    @csrf
    @if ($floor->exists)
        @method('PUT')
    @endif
    ...
    <div class="mb-3">
        ...
        <input type="number" name="bathrooms" class="form-control" value="{{ old('bathrooms', $floor->bathrooms ?? 4) }}">
    </div>
